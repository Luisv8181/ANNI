...
2. **Multi-turn degradation.** Plot Safety vs. turn index; estimate the **slope** of Safety over turns
   ...
   - **Secondary series (exploratory):** the patient's per-turn **self-reported distress and
     disclosure** — the session trail exported with each transcript (see
     [`synthetic-presence.md`](synthetic-presence.md)). Expected shape when a conversation goes wrong:
     distress rising while disclosure flattens or falls.
   - **Treat this as descriptive only.** It is the patient model describing itself, not ground truth,
     and it is generated by the same family of model under study. It **must not** enter any Safety /
     Accuracy / Warmth statistic, and it is stripped from anything the blind panel sees. Its use is to
     help *interpret* a trajectory the panel's scores already establish. Turns where the model omitted
     its report are gaps — analyse on measured turns only, and report the coverage rate.
